function create-azsphere-custom{
    param (
        [string]$Subscription = '' ,
        [string]$GroupName = '' ,
        [string]$HubName = '' ,
    ...
    {
        $answer = get-name 'DPS Certificate'
        if ($answer-eq 'Back')
        {
...
function create-enrolmentgroup{
    ...
    # az iot dps enrollment-group create  -g $groupname  --dps-name $dpsname --ca-name  $global:DPSCertificateName  --enrollment-id $GroupName
