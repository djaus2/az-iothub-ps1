...
namespace back_end_application
{
    class BackEndApplication
    {
        ...
        private static async Task InvokeMethod(int cmd)
        {
            var methodInvocation = new CloudToDeviceMethod("ControlMotor") { ResponseTimeout = TimeSpan.FromSeconds(30) };
            methodInvocation.SetPayloadJson(cmd.ToString());
            methodInvocation.ConnectionTimeout = methodInvocation.ResponseTimeout;
