...
function DoAzureEnity
{
    param (
        [Parameter(Mandatory)]
        [string]$EnityCurrentListString, 
        [Parameter(Mandatory)]
        [string]$EntityCurrent, 
        [Parameter(Mandatory)]
        [string]$ListString, 
        [Parameter(Mandatory)]
        [string]$Index, 
    ...
    write-Host '=== ' + $Title + ' ==='
    write-Host ''
    If ([string]::IsNullOrEmpty($EnityCurrentListString))
    {   
        write-Host 'Getting ' + $Title + 's from Azure'
        $EnityCurrentListString  =  az account list  -o tsv | Out-String  <--- 2Do
    }
    If ([string]::IsNullOrEmpty($EnityCurrentListString))
    {
        $Prompt = 'No ' + $Title + ' found. Have you run Az Login? Exiting.'
        write-Host $Prompt
        Exit

    }
    $Entity = Show-Menu $EnityCurrentListString $Index  $Title $EntityCurrent
